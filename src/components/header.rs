...
pub fn Header(
    ...
    let search_query =
        use_context::<RwSignal<String>>().expect("search_query context not provided");
    let show_settings =
        use_context::<RwSignal<bool>>().expect("show_settings context not provided");
        ...
        if query.is_empty() {
            ...
            posts
                ...
                .and_then(|posts| {
                    posts
                        .into_iter()
                        .find(|post| post.title.to_lowercase().starts_with(&query.to_lowercase()))
                ...
                if is_cmd_or_ctrl && is_k_key {
                    ...
                    if let Some(document) = web_sys::window().and_then(|w| w.document()) {
                        if let Ok(Some(search_input)) =
                            document.query_selector("input[placeholder='Search (⌘K)']")
                        {
                            if let Ok(input_element) =
                                search_input.dyn_into::<web_sys::HtmlInputElement>()
                            {
